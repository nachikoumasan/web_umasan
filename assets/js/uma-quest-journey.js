...
function switchTab(id){active=id;for(const [key,el]of [['today',today],['journal',status],['start',start],['level',level]])el.hidden=key!==id;document.body.dataset.journey=id;nav.hidden=['start','level'].includes(id);$('#journey-menu').hidden=id==='level';nav.querySelectorAll('button').forEach(b=>{if(b.dataset.tab===id)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');});window.scrollTo({top:0,behavior:'instant'});}
